...
- **生效日期**：2026-03-28 起，新开通 VM 默认采用 **多网卡架构（Multi-NIC）**。
- 此前开通的 VM 默认为 **托管路由架构（Managed-PBR）**。
- 如需从 **托管路由架构（Managed-PBR）** 迁移至 **多网卡架构（Multi-NIC）**，请提交工单申请；**暂不支持**从 **多网卡架构（Multi-NIC）** 回退至 **托管路由架构（Managed-PBR）**。

---

## 一、架构总览

| 对比项 | 托管路由架构（Managed-PBR） | 多网卡架构（Multi-NIC） |
|---|---|---|
...
多网卡架构（Multi-NIC）环境下，**必须配置源地址策略路由**，否则回程流量可能走错网卡导致连接异常。

**推荐方式：使用一键脚本（Debian / Ubuntu + Netplan）**

```bash
curl --interface eth1 -fsSL https://github.com/spark-net/sparkvm-docs/raw/main/multi-nic-routing/setup-pbr.py -o setup-pbr.py
...
**手动配置**：参见 [多网卡 Netplan 配置指南](multi-nic-routing/netplan.md)，文档涵盖逐步操作说明和完整的 YAML 示例。

### 4.3 自定义分流路由

多网卡架构（Multi-NIC）的优势在于可灵活控制流量走向：
...
### 从托管路由架构（Managed-PBR）迁移至多网卡架构（Multi-NIC）

1. **提交工单**：说明需要迁移至多网卡架构（Multi-NIC）
2. **准备迁移**：迁移过程中会有网络中断，请提前安排维护窗口
3. **迁移后配置**：参照 [第四章](#四多网卡架构multi-nic使用指南) 完成路由配置
4. **验证连通性**：使用 [4.4 节](#44-验证配置) 的方法验证各线路正常

> ⚠️ **注意**：迁移为单向操作，迁移至多网卡架构（Multi-NIC）后**无法回退**至托管路由架构（Managed-PBR）。请在迁移前充分评估。

---

## 六、常见问题

**Q: 多网卡架构（Multi-NIC）下，不配置策略路由会怎样？**
A: 回程流量可能从错误的网卡发出，导致源 IP 与入站时不一致，远端会丢弃该数据包（非对称路由问题），表现为部分连接超时或不可达。

**Q: 托管路由架构（Managed-PBR）下能否指定某个服务走特定 DIA？**
A: 不能在 VM 内直接控制。需提交工单，由平台在路由器上调整策略路由规则。

**Q: 为什么当前暂不开放托管路由架构（Managed-PBR）新申请？**
A: 目前托管路由架构（Managed-PBR）需要由路由器集中承担策略路由与转发处理，对设备性能压力较大；在大带宽、高连接数场景下，较易出现丢包风险。基于稳定性考虑，当前暂不开放新申请。后续如完成性能调优并通过验证，可能会重新开放。

**Q: 多网卡架构（Multi-NIC）下 DNS 应该怎么配？**
...
A: 03/28 之前开通的 VM 默认为托管路由架构（Managed-PBR）。可通过 `ip addr` 查看网卡数量来确认：若只有一张公网网卡则为托管路由架构（Managed-PBR），多张则为多网卡架构（Multi-NIC）。
